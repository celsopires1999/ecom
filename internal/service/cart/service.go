...
func getCartItemsIDs(items []entity.CartCheckoutItem) ([]int, error) {
...
func checkIfCartIsInStock(cartItems []entity.CartCheckoutItem, products map[int]entity.Product) error {
	if len(cartItems) == 0 {
...
func calculateTotalPrice(cartItems []entity.CartCheckoutItem, products map[int]entity.Product) float64 {
	var total float64
...
func (h *Handler) createOrder(products []entity.Product, cartItems []entity.CartCheckoutItem, userID int) (int, float64, error) {
	...
	// create order record
	orderID, err := h.orderStore.CreateOrder(entity.Order{
		UserID:  userID,
		Total:   totalPrice,
		Status:  "pending",
	...
	for _, item := range cartItems {
		h.orderStore.CreateOrderItem(entity.OrderItem{
			OrderID:   orderID,
